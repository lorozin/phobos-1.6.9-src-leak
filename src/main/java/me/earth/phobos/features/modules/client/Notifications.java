            ...
            if (tickPlayerList.size() > 0) {
                for (EntityPlayer player : tickPlayerList) {
                    ...
                    if (Phobos.friendManager.isFriend(player)) {
                        Command.sendMessage("Player \u00a7a" + player.getName() + "\u00a7r" + " entered your visual range" + (this.coords.getValue() != false ? " at (" + player.posX + ", " + player.posY + ", " + player.posZ + ")!" : "!"), this.popUp.getValue());
                    } else {
                        Command.sendMessage("Player \u00a7c" + player.getName() + "\u00a7r" + " entered your visual range" + (this.coords.getValue() != false ? " at (" + player.posX + ", " + player.posY + ", " + player.posZ + ")!" : "!"), this.popUp.getValue());
                    }
            ...
            if (this.knownPlayers.size() > 0) {
                for (EntityPlayer player : this.knownPlayers) {
                    ...
                    if (this.leaving.getValue().booleanValue()) {
                        if (Phobos.friendManager.isFriend(player)) {
                            Command.sendMessage("Player \u00a7a" + player.getName() + "\u00a7r" + " left your visual range" + (this.coords.getValue() != false ? " at (" + player.posX + ", " + player.posY + ", " + player.posZ + ")!" : "!"), this.popUp.getValue());
                        } else {
                            Command.sendMessage("Player \u00a7c" + player.getName() + "\u00a7r" + " left your visual range" + (this.coords.getValue() != false ? " at (" + player.posX + ", " + player.posY + ", " + player.posZ + ")!" : "!"), this.popUp.getValue());
                        }
                    }
                    return;
                }
    ...
    public void onReceivePacket(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketSpawnObject && this.pearls.getValue().booleanValue()) {
            ...
            if (packet.getEntityID() == 85) {
                Command.sendMessage("\u00a7cPearl thrown by " + player.getName() + " at X:" + packet.getX() + " Y:" + packet.getY() + " Z:" + packet.getZ());
            }
        }
    }
