...
class AutoMinecart
    ...
    private final Setting < Integer > delay = this.register ( new Setting <> ( "Carts" , 20 , 1 , 50 ) );
    ...
    EntityPlayer getTarget ( ) {
        ...
        for (EntityPlayer player : AutoMinecart.mc.world.playerEntities) {
            if ( EntityUtil.isntValid ( player , 6.0 ) || player.isInWater ( ) || player.isInLava ( ) || ! EntityUtil.isTrapped ( player , false , false , false , false , false , false ) || player.getHealth ( ) + player.getAbsorptionAmount ( ) > this.minHP.getValue ( ) )
                continue;
            if ( target == null ) {
                target = player;
