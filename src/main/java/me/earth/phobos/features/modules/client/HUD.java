...
public class HUD
    ...
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "bridger.ware"));
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "Nazi", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PHOBOS: {
                this.renderer.drawString("Fobus" + (this.modeVer.getValue() ? " v1.6.9" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case EARTH: {
                this.renderer.drawString("phobos.uwu" + (this.modeVer.getValue() ? " v1.6.9" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() ? " v1.6.9" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
        ...
        int k = this.renderingUp.getValue() ? (0) : 0;
        if (this.renderingUp.getValue()) {
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Phobos.uwu " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
            ...
            default: {
                text = text + "Sieg Heil " + HUD.mc.player.getDisplayNameString();
                break;
            }
        }
